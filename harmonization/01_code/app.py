...
Quality Check), Step 5 (Comparison Plots), and Step 6 (RCT Regressions).
Additional steps on the combined baseline+endline data will be added as
their own s0N_*.py modules and new pages below, following the same
...
`s04_quality_check.py`, `s05_comparison_plots.py` and
`s06_regressions.py`. It is named `app_harmonization.py` here only to
...
OUTPUTS_REGRESSIONS_DIR = os.path.join(BASE, config["paths"]["outputs_regressions"])
...
pipeline_status = {
    ...
    "6. RCT Regressions": os.path.isdir(OUTPUTS_REGRESSIONS_DIR) and len(os.listdir(OUTPUTS_REGRESSIONS_DIR)) > 0,
    # Future steps on the combined baseline+endline data will be added here.
}
...
with st.sidebar.expander("Expected folder layout"):
    st.code(
        ...
        "├── 05_qualityCheck/\n"
        "├── 06_plots/\n"
        "└── 07_regressions/          (generated by Step 6 below)",
        language="text",
    )
...
# ── Page 6: RCT Regressions ────────────────────────────────────────────────────
elif page == "6. RCT Regressions":
